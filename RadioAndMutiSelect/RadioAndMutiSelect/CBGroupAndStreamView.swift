...
//
//  Created by haitao on 2018/8/13.
//  Copyright © 2018年 haitao. All rights reserved.
//
...
@objc protocol CBGroupAndStreamViewDelegate : NSObjectProtocol {
    ...
    ///   - index: 当前下标
    ///   - groupId: groupId
    @objc optional func currentSelValueWithDelegate(valueStr : String, index : Int, groupId : Int , groundTitle:String)
}
...
class CBGroupAndStreamView: UIView {
    ...
    /// 显示按钮的高度
    public var content_height = 44
    /// 上下按钮之间的间距
    public var content_y = 10
    /// 左右按钮之间的间距
    public var content_x = 10
    /// 文字边框的间距
    public var widthRadio = 60
    /// title默认颜色
    public var content_norTitleColor : UIColor = UIColor.ColorHex("#383838")
    /// title选中颜色
    public var content_selTitleColor : UIColor = UIColor.ColorHex("#46B9A5")
    /// 背景默认原色
    public var content_backNorColor : UIColor = .white
    /// 背景选中颜色
    public var content_backSelColor : UIColor = UIColor.ColorHex("F4FFFD")
    
    public var border_NorColor:UIColor =  UIColor.ColorHex("D9D9D9")
    
    public var border_Selcolor:UIColor =  UIColor.ColorHex("#5BC4B2")
    /// 字体大小
    public var content_titleFont : UIFont = .systemFont(ofSize: 12)
    /// 圆角
    public var content_radius : Int = 44/2
    /// 是否单选，默认 true 是单选
    ...
    private var selectValueArray:Array = Array<Any>()

    override init(frame: CGRect) {
        super.init(frame: frame)
        self.scrollView = UIScrollView()
        self.scrollView.frame = CGRect(x: 0, y: 0, width: UIScreen.main.bounds.size.width, height: self.frame.size.height)
...
//    var groupIndex:Int = 0
    func setDataSource(contetnArr : Array<Any>, titleArr : Array<String>){
        ...
        //设置滚动范围
        scrollView.contentSize = CGSize(width: UIScreen.main.bounds.width, height: frameRect.size.height + frameRect.origin.y)
    }
    //MARK:----设置数据源，创建
    func setupGroupAndStream(content : Array<Any>, titleStr : String, currFrame : CGRect, groupId : Int) -> CGRect{


        var content_totalHeight = CGFloat(0)
        ...
        for (index,value) in content.enumerated() {
            ...
            sender.layer.borderColor = border_NorColor.cgColor
            sender.addTarget(self, action: #selector(senderEvent), for: .touchUpInside)
            //MARK:需修改适配 widthRadio
            let text_width = calcuateLabSizeWidth(str: value as! String, font:content_titleFont, maxHeight: CGFloat(content_height))
            //标签流
            let but_width = text_width + CGFloat(widthRadio)
            //计算每个button的 X
    ...
    private func setDefaultSingleSelect(index : Int , groupId : Int ,value : String, sender : UIButton, content : Array<Any>){
        ...
        if defaultSelSingleIndeArr.isEmpty{
            ...
            if index == defaultSelIndex{
                ...
                sender.layer.borderColor = border_Selcolor.cgColor
...
//        print("----\(sender/.tag)")
    ...
    private func singalSelectEvent(sender : UIButton){
        ...
        if sender.isSelected {
            for (index, _) in tempDetailArr.enumerated(){
                ...
                norSender.layer.borderColor = border_NorColor.cgColor
            }
            ...
            //代理传值
            delegate?.currentSelValueWithDelegate?(valueStr: valueStr, index: sender.tag % 100 - 1, groupId: sender.tag / 100, groundTitle: groundTitle)
        }else{
            sender.backgroundColor = content_backNorColor
            sender.layer.borderColor = border_NorColor.cgColor
        }
    ...
    public func comfirm(){
        ...
        delegate?.confimrReturnAllSelValueWithDelegate?(selArr: saveSelButValueArr, groupArr: saveSelGroupIndexeArr, groundTitle: groundTitle)
